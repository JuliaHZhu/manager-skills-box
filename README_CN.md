...
| **isc-transformation-architect** | -- | ISC集成供应链变革架构师：关注→发明→推行三阶段、SCOR流程设计、组织/IT/流程三环模型。 | 华为供应链管理 |
| **scor-racetrack-improver** | -- | SCOR模型+跑道图五阶段改善：诊断、对标、流程映射、项目优先级排序、供应链转型实施。 | 华为供应链管理 |
| **supply-chain-resilience-guardian** | -- | 供应链韧性与极限生存：芯片战略/备胎机制、BCM业务连续性、战略储备、极限冲击应对。 | 华为供应链管理 |
| **consumer-supply-chain-operator** | -- | 终端/to-C供应链运营：CISC变革、轻库存快反、NPI+EMS外包协同、全球同步上市。 | 华为供应链管理 |
| **integrated-planning-conductor** | -- | 集成计划与S&OP总指挥：计划是龙头、18月滚动计划、产销研协同、策略驱动差异化交付。 | 华为供应链管理 |
| **strategic-sourcing-architect** | -- | 战略采购与采购战略：战略采购五特征、采购战略六步法、采购铁三角、供应商分层分级。 | 华为供应链管理 |
| **supply-chain-digitizer** | -- | 供应链数字化转型：数字化→数智化→数治化三阶段，数字孪生，灵鲲/灵蜂双层智能架构。 | 华为供应链管理 |
| **isc-plus-digital-transformer** | -- | ISC+数字化供应链变革：六大转变、轻量级IT架构、八个子项目群、ROADS体验标准。 | 华为供应链管理 |
...
| **mrp-master-scheduler** | -- | MRP三层计划排产师：MDS-MPS-MRP主计划制定与调整、版本切换控制、例外信息处理、预缺料管理。 | 华为计划管理 |
| **inventory-plan-controller** | -- | 库存计划与物控管理：最小-最大值库存计划、ABC分类、死料/低周转分析、缺料解决、日清日结。 | 华为计划管理 |
| **demand-supply-balancer** | -- | S&OP供需平衡与需求评审：无约束预测、供产销评审会、需求波动平滑、研发/内部需求评审。 | 华为计划管理 |
| **bom-eco-guardian** | -- | BOM工程数据与ECO管控：产品结构树、项目模板、清单类型、工程更改单、计划百分比维护。 | 华为计划管理 |
| **planning-parameter-engineer** | -- | 计划参数与属性配置：提前期、批量、安全库存、ABC固定供应天数、计划员属性、内部订单配置。 | 华为计划管理 |
| **supply-chain-planning-architect** | -- | 供应链计划体系设计：MRP/JIT/TOC方法论选择、计划组织架构、ISC变革、TOM全面订单管理模型。 | 华为计划管理 |
| **inventory-health-diagnoser** | -- | 库存健康诊断与结构优化：库存周转率、合理结构比例、不良库存根因分析、死料/低周转物料处置。 | 华为计划管理 |
| **version-switch-controller** | -- | 版本切换管理：平滑/强行/权衡三种策略、五环节协同、跨部门同步齐步走、呆死料控制目标。 | 华为计划管理 |
| **bottleneck-material-allocator** | -- | 瓶颈物料分配与ATP分析：分配三原则、ATP正向模拟、灵敏度分析、齐套性检查。 | 华为计划管理 |
| **production-control-operator** | -- | 生产物控与排产发货策略：推式/拉式排产、任务令排队规则、发货策略、缺料分层分级、日清日结。 | 华为计划管理 |
| **tom-order-manager** | -- | TOM全面订单管理：四方阵闭环、S&OP资源合理化、业务规则体系、产品组合预测分解、ATP调配。 | 华为计划管理 |
| **rd-logistics-planner** | -- | 研发物流与新品供货风险：研发物流四阶段、器件选型五维评估、提前供货五维风险、研发自购管理。 | 华为计划管理 |
...
| **virtual-equity-governance-architect** | -- | 虚拟股权治理架构：工会持股平台、五权分离、创始人1%控制、控股公司防火墙设计。 | 华为股权激励 |
| **tup-incentive-architect** | -- | TUP激励设计：5年递延递增单元、九定流程、劳动所得与资本所得比例控制、算法化分配。 | 华为股权激励 |
| **equity-lifecycle-strategist** | -- | 股权激励生命周期策略：按初创/成长/成熟/危机阶段匹配ESOP/虚拟股/TUP/ATUP工具。 | 华为股权激励 |
| **equity-incentive-pain-doctor** | -- | 股权激励痛点诊断：14种失败模式、五类股东匹配、90天恢复路线图。 | 华为股权激励 |
| **atup-ecosystem-integrator** | -- | ATUP生态整合：将获取分享制外延至外部合作伙伴、平台价值池、2年伙伴周期。 | 华为股权激励 |
| **striver-ownership-culture** | -- | 奋斗者股权文化：三层共同体、年度奋斗者认证、故事驱动文化传承。 | 华为股权激励 |
| **equity-10d-designer** | -- | 10D股权激励设计模型：定目的/定架构/定对象/定模式/定数量/定价格/定时间/定来源/定条件/定机制。 | 华为股权激励 |
| **virtual-equity-architect** | -- | 虚拟受限股与TUP计划设计：持股平台、定价、兑现、退出与治理架构。 | 华为股权激励 |
| **knowledge-capitalization-strategist** | -- | 知识资本化框架：将智力劳动转化为股权，九大激励机制，奋斗者文化根基。 | 华为股权激励 |
| **equity-governance-builder** | -- | 员工持股治理架构：持股平台、员工代表会、轮值CEO、创始人否决权设计。 | 华为股权激励 |
| **stage-equity-matcher** | -- | 企业阶段匹配股权激励模式：实股/虚拟股/TUP/期权与初创/成长/成熟阶段对齐。 | 华为股权激励 |
| **equity-saturation-governor** | -- | 股权饱和与动态调整：饱和配股线、奖励配股、绩效降级回购、退休保留、退出回购。 | 华为股权激励 |
...
| **project-centric-org-transformer** | -- | 以项目为中心的组织转型：五大构件（组织定义/授权/资源买卖/评价/IT）、三大关键要素、铁三角。 | 华为项目管理 |
| **tob-big-project-strategist** | -- | ToB大项目赢单策略：五阶段漏斗、三板斧+一纸禅、营销四要素、LTC端到端、标前引导。 | 华为项目管理 |
| **project-standard-toolkit** | -- | 项目管理标准工具箱：十大模板、六步一法、十张图表（甘特图/燃尽图/WBS/RACI/PERT等）。 | 华为项目管理 |
| **project-ceo-cultivator** | -- | 项目CEO培养体系：HEROS模型、懂经营重质量领导力三软技能、三阶段成长、721+AAR训战。 | 华为项目管理 |
| **project-team-communicator** | -- | 项目沟通与团队领导：沟通三原则、PL管理、C8协同、项目HRBP“人”字模型。 | 华为项目管理 |
| **project-process-controller** | -- | 项目流程与风险管控：六阶段45标准动作、IPD+CMMI+敏捷融合、PQA过程审计、四风险响应、变更管理。 | 华为项目管理 |
...
| **project-style-formula** | -- | 华为十四字项目管理风格：一魂三动五意聚，两极四世八方齐。项目经理个人管理章法与软技能。 | 华为项目管理 |
| **overseas-project-entrepreneur** | -- | 海外项目创业式管理：本地化队伍建设、信任授权、客户标准唯一尺度、奋斗者保障。 | 华为项目管理 |
| **project-commercial-closer** | -- | 营赢盈项目经营闭环：价值主张对准痛点、战略合作伙伴升级、售前售后拉通盈利。 | 华为项目管理 |
| **project-time-rhythm-master** | -- | 项目工作韵律管理：打扰诊断、专注时间块、忙碌公示、协作时段协商。 | 华为项目管理 |
| **project-root-capability-builder** | -- | 项目管理根能力建设：价值交付正循环、降解思维、HEROS模型、PMO治理、四个一体系。 | 华为项目管理 |
| **client-decision-navigator** | -- | 客户决策链穿透：权力地图、鱼骨图、SPIN销售法、拜访策划、AAR复盘。 | 华为项目管理 |
...
| **business-review-facilitator** | -- | 经营分析会主持方法论：从差距出发、数据化根因分析、建立流程防复发。三大通病诊断与解决。 | 华为经营管理 |
| **process-driven-org-builder** | -- | 流程化组织建设：业务流→流程→数据→IT→质量→组织六层集成设计方法论。 | 华为经营管理 |
| **frontline-empowerment-architect** | -- | 一线授权与矩阵组织设计：授权四大原则、四阶段成熟度、海军陆战队模式、授权后监督。 | 华为经营管理 |
| **matrix-organization-architect** | -- | 矩阵型组织设计：强/平衡/弱矩阵三种形态、1+N汇报、二元结构、委员会治理、人才复用。 | 华为经营管理 |
| **business-transformation-navigator** | -- | 业务转型导航：华为终端七大战略决策案例、从零起飞奖、平台整合、战略耐心。 | 华为经营管理 |
| **strength-based-team-builder** | -- | 基于优势建团队：歪瓜裂枣人才哲学、狼狈组织搭配、互补角色胜过完美个人。 | 华为经营管理 |
| **ren-business-philosophy** | -- | 任正非经营哲学十二条：活着、洗盐碱地、红军蓝军、深淘滩低作堰、力出一孔、灰度等。 | 华为经营管理 |
| **execution-craftsman** | -- | 华为执行工匠：SMART目标纪律、上层作势基层做实、对事负责制、四化管理、压强原则。 | 华为经营管理 |
| **goal-framework-navigator** | -- | 目标框架导航：MBO目标解码vs分解、BSC四维度、OKR+CFR、绩效考核程序公正。 | 华为经营管理 |
| **decision-balance-master** | -- | 决策平衡术：成长vs利润、改良vs革命、无生命管理、领先半步、独裁vs民主六大平衡。 | 华为经营管理 |
| **management-twist-engineer** | -- | 拧麻花管理机制设计：前后拧、上下拧、里外拧三种张力机制，解决规模与效益矛盾。 | 华为经营管理 |
| **matter-accountability-designer** | -- | 对事负责制体系设计：以流程和结果替代对人负责，消除事事请示文化。 | 华为经营管理 |
| **responsibility-center-operator** | -- | 责任中心经营体系：投资/利润/成本/费用中心划分、成本价内部结算、下放经营权+加强监控权。 | 华为组织管理 |
| **org-synergy-architect** | -- | 组织协同架构：流程拉通、矩阵结构、双计双考、机关资源化/资源市场化、利益驱动协同。 | 华为组织管理 |
| **result-orientation-guardian** | -- | 责任结果导向落地：正向考核+逆向考事、危机层层分解、客户-流程-绩效三环、分层考核。 | 华为组织管理 |
| **anti-involution-diagnoser** | -- | 内卷与内耗诊断：30种内卷行为识别、十大内耗框架、组织健康审视、从管控到平台激发。 | 华为组织管理 |
| **consulting-value-maximizer** | -- | 咨询项目价值最大化：信任前置、价格不限制价值、里程碑管理、知识内化、长期共赢。 | 华为组织管理 |
| **simple-org-culture-builder** | -- | 简单组织与文化构建：建制优于胜任、三机制（分权/分钱/评价）、价值观行为化、契约精神。 | 华为组织管理 |
